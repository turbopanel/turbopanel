...
test('populated when the key is present; signing secret optional; version pinned by default', () => {
  assertEquals(resolveBillingConfig({ [STRIPE_SECRET_KEY_ENV]: ' sk_test_1 ' }), {
    secretKey: 'sk_test_1',
  ...
  assertEquals(
    ...
    { secretKey: 'sk_test_1', webhookSigningSecret: 'whsec_1', apiVersion: '2030-01-01' },
  )
  // A blank override falls back to the pin rather than sending an empty header.
  assertEquals(
    resolveBillingConfig({ [STRIPE_SECRET_KEY_ENV]: 'sk', [STRIPE_API_VERSION_ENV]: ' ' })?.apiVersion,
    DEFAULT_STRIPE_API_VERSION,
  )
})
