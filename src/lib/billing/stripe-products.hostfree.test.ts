...
function withPrice(overrides: Record<string, unknown>): ProviderProduct {
  return summarizeStripeProduct({ ...PRODUCT_S3, default_price: { ...PRICE_S3, ...overrides } })
}
...
test('an unexpanded default_price (bare id) reads as no price; a missing name falls back to the id; non-string metadata is dropped', () => {
  const bare = summarizeStripeProduct({ ...PRODUCT_S3, default_price: 'price_s3' })
  assertEquals(bare.defaultPrice, null)
  assertEquals(summarizeStripeProduct({ ...PRODUCT_S3, default_price: null }).defaultPrice, null)
  assertEquals(summarizeStripeProduct({ id: 'prod_x', active: true }).name, 'prod_x')
  assertEquals(summarizeStripeProduct({ id: 'prod_x', active: 'yes', livemode: 1 }).active, false)
  assertEquals(summarizeStripeProduct({ id: 'prod_x', livemode: 1 }).livemode, false)
  assertEquals(summarizeStripeProduct({ id: 'prod_x', metadata: { a: '1', b: 2, c: null } }).metadata, { a: '1' })
  assertEquals(summarizeStripeProduct({ id: 'prod_x', metadata: 'nope' }).metadata, {})
...
test('summarizeStripePrice tolerates a one-time price (no recurring block) and non-finite amounts', () => {
  const oneTime = summarizeStripePrice({ id: 'price_once', active: true, type: 'one_time', currency: 'usd', unit_amount: 500 })
  assertEquals([oneTime.type, oneTime.interval, oneTime.intervalCount, oneTime.billingScheme, oneTime.taxBehavior], ['one_time', null, null, null, null])
  assertEquals(summarizeStripePrice({ id: 'p', unit_amount: null }).unitAmount, null)
  assertEquals(summarizeStripePrice({ id: 'p', unit_amount: '1000' }).unitAmount, null)
...
test('suggestedLabel reads the tier metadata case-insensitively and trimmed; an invalid label is null', () => {
  const suggested = (value: unknown) => summarizeStripeProduct({ id: 'prod_x', metadata: { [PRODUCT_TIER_METADATA_KEY]: value } }).suggestedLabel
  ...
  assertEquals(summarizeStripeProduct({ id: 'prod_x', metadata: { tier: 'S3' } }).suggestedLabel, null)
})
...
test('a well-formed monthly per-unit usd price with a tax behavior verifies clean', () => {
  ...
  assertEquals(verifyStripeProduct(GOOD), { ok: true, product: GOOD, failures: [] })
  assertEquals(productVerificationFailures(withPrice({ tax_behavior: 'inclusive' })), [])
})
...
test('productVerificationFailures names every reason, one per check', () => {
  ...
  assertEquals(only(summarizeStripeProduct({ ...PRODUCT_S3, active: false })), 'product is archived (active=false)')
  assertEquals(only(withPrice({ active: false })), 'default price is archived (active=false)')
  assertEquals(only(withPrice({ type: 'one_time' })), 'default price type one_time ≠ recurring')
  assertEquals(only(withPrice({ recurring: { interval: 'year', interval_count: 1 } })), 'recurring.interval year ≠ month')
  assertEquals(only(withPrice({ recurring: { interval: 'month', interval_count: 3 } })), 'recurring.interval_count 3 ≠ 1')
  assertEquals(only(withPrice({ billing_scheme: 'tiered' })), 'billing_scheme tiered ≠ per_unit')
  assertEquals(only(withPrice({ currency: 'eur' })), 'currency eur ≠ usd')
  assertEquals(only(withPrice({ unit_amount: null })), 'default price has no unit_amount')
  const noTaxAnywhere = 'no tax behaviour resolves for this price: it is unspecified and Stripe Tax ' +
...
test('no default price is the one failure that short-circuits the rest; other failures accumulate', () => {
  const noPrice = summarizeStripeProduct({ ...PRODUCT_S3, active: false, default_price: null })
  ...
  assertEquals(productVerificationFailures(summarizeStripeProduct({ ...PRODUCT_S3, default_price: 'price_s3' })), [
    'product has no default price; set one in the Dashboard',
  ])
  const many = withPrice({ active: false, currency: 'eur', recurring: { interval: 'year', interval_count: 2 }, billing_scheme: 'tiered' })
...
test('listStripeProducts asks for every active product with its default price expanded', async () => {
  const client = createStripeClientDouble(() => ({ object: 'list', data: [PRODUCT_S3, { ...PRODUCT_S3, id: 'prod_s5', name: 'S5', metadata: {} }] }))
  const products = await listStripeProducts(client)
  assertEquals(products.map((p) => [p.id, p.suggestedLabel]), [['prod_s3', 'S3'], ['prod_s5', null]])
  assertEquals(client.calls.length, 1)
  const [call] = client.calls
  assertEquals([call?.method, call?.path, call?.idempotencyKey], ['GET', '/v1/products', null])
  assertEquals(formOf(call!, 'active'), 'true')
  assertEquals(call?.form.some(([key, value]) => key.startsWith('expand[') && value === 'data.default_price'), true)
...
test('getStripeProduct fetches one product by id with the default price expanded, encoding the id', async () => {
  const client = createStripeClientDouble((call) => {
    ...
    if (call.path === `/v1/products/${encodeURIComponent('prod/odd id')}`) return { ...PRODUCT_S3, id: 'prod/odd id' }
  ...
  assertEquals(call?.form.some(([key, value]) => key.startsWith('expand[') && value === 'default_price'), true)
  assertEquals(call?.form.some(([key]) => key === 'active'), false)
  assertEquals((await getStripeProduct(client, 'prod/odd id')).id, 'prod/odd id')
})

test('createStripeGateway is the stripe provider and routes each method to the client', async () => {
  const client = createStripeClientDouble((call) => (call.path === '/v1/products' ? [PRODUCT_S3] : PRODUCT_S3))
  ...
  assertEquals(client.calls.map((call) => call.path), ['/v1/products', '/v1/products/prod_s3'])
})
...
const ACCOUNT_EXCLUSIVE: ProviderTaxDefaults = { taxBehavior: 'exclusive', status: 'active' }
...
test('an unspecified price passes when the account sets a default tax behaviour', () => {
  ...
  assertEquals(productVerificationFailures(unspecified, { taxBehavior: 'inclusive', status: 'active' }), [])
...
test('an unspecified price still fails when the account names no default either', () => {
  const unspecified = withPrice({ tax_behavior: 'unspecified' })
  for (const defaults of [NO_TAX_DEFAULTS, { taxBehavior: 'unspecified', status: 'active' }]) {
...
test('summarizeStripeTaxDefaults reads the defaults block and tolerates a bare object', () => {
  assertEquals(
    summarizeStripeTaxDefaults({ status: 'active', defaults: { tax_behavior: 'exclusive' } }),
    { taxBehavior: 'exclusive', status: 'active' },
  )
  // Stripe's own example returns a null behaviour under an active account.
  assertEquals(
    summarizeStripeTaxDefaults({ status: 'active', defaults: { tax_behavior: null } }),
    { taxBehavior: null, status: 'active' },
  )
  assertEquals(summarizeStripeTaxDefaults({}), { taxBehavior: null, status: null })
})

test('getStripeTaxDefaults reads /v1/tax/settings and degrades to no default on a Stripe error', async () => {
  const client = createStripeClientDouble(() => ({ status: 'active', defaults: { tax_behavior: 'exclusive' } }))
  assertEquals(await getStripeTaxDefaults(client), { taxBehavior: 'exclusive', status: 'active' })
...
test('the gateway exposes the account tax defaults', async () => {
  const client = createStripeClientDouble(() => ({ status: 'active', defaults: { tax_behavior: 'exclusive' } }))
  assertEquals(await createStripeGateway(client).getTaxDefaults(), { taxBehavior: 'exclusive', status: 'active' })
})
