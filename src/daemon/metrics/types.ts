 ...
 * `queryEntitySeries` / `queryEntityIdsSeen` — every {@link HostedFamily}
 ...
 * `HOST_METRICS_METRIC_DESCRIPTORS` scoped to a `host.*` entity scope
 * (`host.cpu.busyPercent`, `host.memory.usedBytes`, etc.) — see
...
/**
 * Multi-entity, multi-metric series query for one {@link PerEntityHostedFamily}.
 *
 ...
 *
 * `metrics` are bare field names (`HostMetricsMetricDescriptor.fieldName`)
 ...
 * devices are embedded directly into `host.io` (see `field-map.ts`'s
 ...
 * fields — see `field-map.ts`'s `HOST_IO_EMBEDDED_NIC_FIELDS`) from
...
export type EntitySeriesPoint = {
   ...
   * neither aggregates across buckets (see
   * `query/derived-metrics.ts`'s `computeIngressDerivedValues`). Stores never
   * populate this; `metrics-routes-helpers.ts`'s `withIngressDerivedValues`
   * attaches it on the `/servers/:id/metrics/series` read path, so it is
   * absent on every other family.
   *
   * Each figure is computed from this point's own `values`, so it is `null`
 ...
 * `DisabledServerMetricsStore` needs no changes, and
 * `UnavailableServerMetricsStore` (`store-selection-core.ts`) can add
