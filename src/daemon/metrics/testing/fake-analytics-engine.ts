 ...
 * `timestamp`, `blob1..blob{AE_BLOB_COUNT}`,
 * `double1..double{AE_DOUBLE_COUNT}`, plus a literal `_sample_interval`
 ...
 * `CloudflareAnalyticsEngineServerMetricsStore` write path and read back
 * through the real `queryXViaSqlApi` functions (`sql-api.ts`) by
 ...
 * have no DuckDB-native equivalent under the same name; `installAeShims`
...
export type FakeAnalyticsEngine = {
  /** Pass to `CloudflareAnalyticsEngineServerMetricsStore`'s constructor. */
  dataset: AnalyticsEngineDatasetLike
  /** Pass as `{ sql: sqlConfig }` to the store, or straight to a `queryXViaSqlApi` call. */
...
async function installAeShims(connection: FakeAeConnectionLike): Promise<void> {
...
function normalizeForJson(value: unknown): unknown {
  ...
  if (value !== null && typeof value === 'object') {
    ...
    for (const [key, inner] of Object.entries(value as Record<string, unknown>)) {
      out[key] = normalizeForJson(inner)
    }
    return out
  }
  return value
...
export async function createFakeAnalyticsEngine(options?: {
  dataset?: string
}): Promise<FakeAnalyticsEngine> {
  const datasetName = options?.dataset ?? AE_DATASET_NAME
  ...
  const dataset: AnalyticsEngineDatasetLike = {
    writeDataPoint(event) {
      const doubles = new Array<number>(AE_DOUBLE_COUNT).fill(0)
      ;(event.doubles ?? []).forEach((value, i) => {
        if (i < AE_DOUBLE_COUNT) doubles[i] = value
      })
      const blobs = new Array<string>(AE_BLOB_COUNT).fill('')
      ;(event.blobs ?? []).forEach((value, i) => {
        if (i < AE_BLOB_COUNT) blobs[i] = value
      })
      pending.push({
  ...
  async function flush(): Promise<void> {
    ...
    const columns = [
      ...
      `"timestamp"`,
      '_sample_interval',
      ...Array.from({ length: AE_BLOB_COUNT }, (_, i) => `blob${i + 1}`),
      ...Array.from({ length: AE_DOUBLE_COUNT }, (_, i) => `double${i + 1}`),
    ]
