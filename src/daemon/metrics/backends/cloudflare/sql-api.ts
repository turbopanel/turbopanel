 ...
 *
 * Per-entity series (`queryEntitySeriesViaSqlApi`) for the paged
 ...
 * (`parsePagedEntitySeriesRows`): each row's `blob10` is split to learn
 ...
 * share the same `sourceKind`), so `queryEntitySeriesViaSqlApi` groups
...
/**
 * Row-kind + schema-version discriminators for the current dataset
 * (`kind` is `"metrics"` / `"event"` / `"status"` — see `field-map.ts`'s
 * `AE_BLOB_KIND_INDEX`).
 */
...
/**
 * AE SQL literal matching write-path `AE_MISSING_METRIC_SENTINEL`
...
/**
 * Host-singleton entity scopes queryable via `queryHostSeries` — `host.*`
 * plus the four host-wide singleton families that carry no entity identity of
 * their own: the merged `diagnostics` scalar scope, `managed.router`,
 * `managed.storage` and `managed.docker`. All four live here rather than on
 * the per-entity path for the same reason: a host runs exactly one shared
 * ingress router, has exactly one storage picture and one Docker daemon, so
 * there is no `sourceId` to group by and each row resolves by family + double
 * index.
 */
...
function buildMetricEventsSql(
  ...
  return [
    ...
    `  ${statusReasonColumn()} AS severity,`,
    `  ${blobColumn(AE_BLOB_EVENT_ENTITY_ID_INDEX)} AS entity_id,`,
...
/**
 * `GROUP BY bucket, entity_id` in {@link buildSingleRowEntitySeriesSql}
 ...
 * `aggregateExpressionForDescriptor` — so unlike the paged path, there is
 * no cross-row recombination to do here, just a direct row-to-point mapping.
 */
function parseSingleRowEntitySeriesRows(
  fields: readonly string[],
  aliases: readonly string[],
  entityIds: readonly string[],
  data: Array<Record<string, unknown>>,
...
function accumulatePagedEntitySlot(
  perEntity: Map<string, Map<number, BucketEntityAccumulator>>,
  entityId: string,
