...
function writeCountFor(
  ...
  const store = new CloudflareAnalyticsEngineServerMetricsStoreV4(fake.dataset)
...
// ---------------------------------------------------------------------------
// writeSample / writeStatusEvent delegate to field-map-v4.ts exactly
// ---------------------------------------------------------------------------

it('writeSample: calls match buildMetricsDataPointsV4 exactly, fire-and-forget', () => {
  const fake = createFakeDataset()
  const store = new CloudflareAnalyticsEngineServerMetricsStoreV4(fake.dataset)
  const sample = buildSample({ gpus: [mkGpu('gpu0')] })
  store.writeSample(sample)
  assertEquals(fake.calls, buildMetricsDataPointsV4(sample))
})
...
it('writeStatusEvent: exactly one writeDataPoint, matching buildStatusDataPointV4', () => {
  const fake = createFakeDataset()
  const store = new CloudflareAnalyticsEngineServerMetricsStoreV4(fake.dataset)
  const event = {
    serverId: '11111111-2222-4333-8444-555555555555',
  ...
  assertEquals(fake.calls[0], buildStatusDataPointV4(event))
})

// ---------------------------------------------------------------------------
// v4 read-path wiring — no `sql` config reports `available: false` with the
// correct empty shape; a configured `sql` client delegates to sql-api-v4.ts.
// ---------------------------------------------------------------------------
