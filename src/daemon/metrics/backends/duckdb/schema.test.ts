...
it('buildSchemaStatements emits idempotent DDL for every v4 table', () => {
  ...
  assertEquals(
    joined.includes(`ON ${CPU_HOTSPOT_SAMPLES_TABLE} (server_id, core_id, sampled_at)`),
    true
  )
  assertEquals(
    joined.includes(`ON ${CPU_CORE_SAMPLES_TABLE} (server_id, core_id, sampled_at)`),
    true
  )
