 ...
 * failure degrades to `UnavailableServerMetricsStoreV5` (reads reject with
 ...
 */
function resolveDenoServerMetricsStoreV5(
  ...
  try {
    ...
    return new UnavailableServerMetricsStoreV5(`DuckDB metrics store failed to open: ${message}`)
  }
}
...
export function resolveServerMetricsStoreV5(
  ...
  if (input.runtime === 'workers') {
    return resolveWorkersServerMetricsStoreV5(input)
  }
  return resolveDenoServerMetricsStoreV5(input)
}
