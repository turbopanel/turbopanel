...
export class DaemonCellObject {
  ...
  constructor(ctx: DurableObjectState, env: CloudflareBindings) {
    ...
    setServerStatusEventSink(
      resolveServerMetricsStore({
        runtime: "workers",
        analyticsEngine:
          (env as { SERVER_METRICS?: AnalyticsEngineDatasetLike })
            .SERVER_METRICS,
      }),
    );
  ...
  #readSchemaVersion(): number | null {
    try {
      const pragmaRow = readFirstSqlRow(
        this.#sql("ensure-schema", "PRAGMA user_version"),
      );
      if (pragmaRow) {
  ...
  async #projectConnected(
    ...
    await this.#withProjectionDb("connect", serverId, async (db) => {
      ...
      await enqueueLatestRecordedCapabilityPlan(
        ...
        serverId,
        (envelope) => {
          this.#enqueue(serverId, envelope);
        },
        "hosted",
      );
      if (this.#isDaemonDebug()) {
  ...
  async #clearUpdateStatus(
    ...
    for (const row of inFlightCursor) {
      ...
      const stale = opts?.allowStale &&
        this.#isStaleInFlightUpdate(createdAt, opts);
      if (stale) {
  ...
  #isStaleInFlightUpdate(
    ...
    if (
      opts.targetCommit && opts.currentCommit &&
