...
test("parseTierCreateBody accepts SX with no product and refuses SX with one", () => {
  assertEquals(parseTierCreateBody({ label: "SX" }), {
    label: "SX",
    providerProductId: null,
  });
  assertEquals(parseTierCreateBody({ label: "sx", providerProductId: null }), {
    label: "SX",
    providerProductId: null,
  });
  assertEquals(parseTierCreateBody({ label: "SX", providerProductId: "" }), {
    label: "SX",
    providerProductId: null,
  });
  const refused = parseTierCreateBody({
    label: "SX",
    providerProductId: "prod_sx",
  });
  assertEquals("error" in refused, true);
  assertEquals(
    "error" in refused && refused.error.includes("takes no product"),
    true,
  );
});
...
test("parseTierCreateBody refuses a label off the ladder, naming every label it takes", () => {
  for (
    const body of [{}, { label: "S9" }, { label: "gold" }, { label: 3 }, {
      label: "",
    }]
  ) {
    ...
    if ("error" in refused) {
      for (const entry of LADDER) {
        assertEquals(refused.error.includes(entry.label), true);
      }
    }
  }
});
...
test("parseTierCreateBody refuses a priced label with a missing, blank, or non-string product", () => {
  for (
    const body of [{ label: "S3" }, { label: "S3", providerProductId: null }, {
      label: "S3",
      providerProductId: "   ",
    }]
  ) {
    ...
    assertEquals(
      "error" in refused &&
        refused.error.includes("S3 needs the provider product"),
      true,
      JSON.stringify(body),
    );
  }
...
test("parseTierPatchBody carries only the keys that were sent", () => {
  ...
  assertEquals(parseTierPatchBody({ providerProductId: "prod_other" }), {
...
test("parseTierPatchBody reads null and blank as clearing the product", () => {
  assertEquals(parseTierPatchBody({ providerProductId: null }), {
    providerProductId: null,
  });
  assertEquals(parseTierPatchBody({ providerProductId: "" }), {
    providerProductId: null,
  });
  assertEquals(parseTierPatchBody({ providerProductId: "   " }), {
    providerProductId: null,
...
test("parseTierPatchBody refuses wrong types and names unknown keys", () => {
  assertEquals(parseTierPatchBody({ providerProductId: 7 }), {
    error: "providerProductId must be a string",
  });
  assertEquals(parseTierPatchBody({ isActive: "yes" }), {
    error: "isActive must be a boolean",
  });
  assertEquals(parseTierPatchBody({ isActive: null }), {
    error: "isActive must be a boolean",
  });
  // Identity columns and the old entitlement columns are not patchable.
  assertEquals(parseTierPatchBody({ label: "S4" }), {
...
test("serializeAdminTier reports null entitlements for a label the ladder no longer knows", () => {
  const out = serializeAdminTier(tierRow({ label: "S0", rank: 0 }), {
    seats: 0,
    servers: 0,
  });
...
test("serializeLadderEntry carries rank, custom flag, list price and the slot budgets", () => {
  ...
  const sx = serializeLadderEntry(
    LADDER.find((entry) => entry.label === "SX")!,
  );
  assertEquals(sx.isCustom, true);
  assertEquals(sx.listPriceCents, null);
});

test("ladderProductExpectation is the priced rung; SX has none", () => {
  assertEquals(ladderProductExpectation("S3"), {
    label: "S3",
    listPriceCents: 1000,
  });
  assertEquals(ladderProductExpectation("S1"), {
    label: "S1",
    listPriceCents: 500,
  });
  assertEquals(ladderProductExpectation("SX"), null);
});
...
test("ladderWithRows lists every ladder label in rank order with the id of the row that maps it", () => {
  ...
  assertEquals(out.map((entry) => entry.label), [
    "S1",
    "S2",
    "S3",
    "S4",
  ...
  assertEquals(out.map((entry) => entry.tierId), [
    null,
    null,
    S3_ID,
    null,
  ...
  assertEquals(
    ladderWithRows([]).every((entry) => entry.tierId === null),
    true,
  );
});
...
test("serializeProduct reports a product with no default price and an unbound one", () => {
  ...
  assertEquals(out.verification, {
    ok: false,
    failures: ["product has no default price; set one in the Dashboard"],
  });
  // The failures are copied, not shared with the caller's array.
  const failures = ["x"];
  const copied = serializeProduct(
    bare,
    { ok: false, product: bare, failures },
    null,
  );
  failures.push("y");
