...
function product(overrides: Record<string, unknown> = {}, price: Record<string, unknown> = {}) {
  return {
    id: 'prod_s3',
    object: 'product',
    active: true,
 ...
 */
function catalogue(products: Record<string, unknown>[], accountTaxBehavior: string | null = null) {
  return (call: StripeCall) => {
    if (call.path === '/v1/tax/settings') {
      return { object: 'tax.settings', status: 'active', defaults: { tax_behavior: accountTaxBehavior } }
    }
    if (call.path === '/v1/products') return { object: 'list', data: products, has_more: false }
...
async function buildApp(opts: Readonly<{
  ...
  const client = createStripeClientDouble(opts.respond ?? catalogue([product()]))
...
function get(app: Hono<AppEnv>, cookie: string, path: string): Promise<Response> {
  return Promise.resolve(app.request(path, { method: 'GET', headers: { cookie } }))
}
...
type CreateResponse = { tier: SerializedTier; verification: { ok: boolean; failures: string[] } | null }

test('every tier route answers 503 when billing is off, before touching the provider', async () => {
  const { app, cookie, client, db } = await buildApp({ billing: false, tiers: [tierRow()] })
  const attempts: [string, () => Promise<Response>][] = [
    ['GET /tiers', () => get(app, cookie, '/tiers')],
    ['GET /tiers/products', () => get(app, cookie, '/tiers/products')],
    ['POST /tiers', () => post(app, cookie, '/tiers', { label: 'S3', providerProductId: 'prod_s3' })],
    ['POST /tiers/verify', () => post(app, cookie, '/tiers/verify')],
    ['PATCH /tiers/:id', () => patch(app, cookie, `/tiers/${S3_ID}`, { isActive: false })],
    ['POST /tiers/:id/deactivate', () => post(app, cookie, `/tiers/${S3_ID}/deactivate`)],
    ['POST /tiers/:id/verify', () => post(app, cookie, `/tiers/${S3_ID}/verify`)],
  ]
  for (const [name, attempt] of attempts) {
    const res = await attempt()
    assertEquals(res.status, 503, name)
    assertEquals((await jsonBody<{ error: string }>(res)).error, 'billing_not_configured', name)
  }
...
test('GET /tiers lists every row with its references and the ladder with a tierId per mapped label', async () => {
  const { app, cookie, client } = await buildApp({
    ...
    seats: [{ id: 'seat-1', subscriptionId: 'sub-row', tierId: S3_ID, providerItemId: 'si_1', providerPriceId: 'price_s3', quantity: 2, createdAt: NOW, updatedAt: NOW }],
    servers: [
      { id: 'srv-1', organizationId: ORG, createdAt: NOW, updatedAt: NOW, name: 'a', metadata: null, assignedTierId: S3_ID },
      { id: 'srv-2', organizationId: ORG, createdAt: NOW, updatedAt: NOW, name: 'b', metadata: null, assignedTierId: null },
    ],
  })
  ...
  const body = await jsonBody<{
    ...
    ladder: { label: string; rank: number; isCustom: boolean; tierId: string | null; entitlements: Record<string, number> }[]
  }>(res)
  ...
  assertEquals(body.ladder.map((entry) => entry.label), ['S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'S7', 'SX'])
  assertEquals(body.ladder.find((entry) => entry.label === 'S3')?.tierId, S3_ID)
  assertEquals(body.ladder.find((entry) => entry.label === 'SX')?.tierId, SX_ID)
...
test('GET /tiers/products lists the provider catalogue with inline verification and the tier already bound', async () => {
  const { app, cookie, client } = await buildApp({
    ...
    respond: catalogue([
      product(),
      product({ id: 'prod_s4', name: 'S4', metadata: { turbopanel_tier: 's4' } }, { id: 'price_s4', unit_amount: 1500, tax_behavior: 'unspecified' }),
      product({ id: 'prod_bare', name: 'Bare', metadata: {}, default_price: null }),
    ]),
  })
  ...
  assertEquals(body.products.map((entry) => entry.id), ['prod_s3', 'prod_s4', 'prod_bare'])
  ...
  assertEquals(s4?.verification.failures.some((f) => f.includes('tax_behavior')), true)

  assertEquals(bare?.suggestedLabel, null)
  assertEquals(bare?.defaultPrice, null)
  assertEquals(bare?.verification.failures, ['product has no default price; set one in the Dashboard'])
  ...
  assertEquals(client.calls.map((call) => call.path), ['/v1/products', '/v1/tax/settings'])
...
test('GET /tiers/products maps a provider list failure to 502 product_lookup_failed in our words', async () => {
  ...
  assertEquals(body.message, 'the provider could not be reached to list products')
...
test('POST /tiers verifies the product first and writes nothing when it does not conform', async () => {
  const { app, cookie, db, client } = await buildApp({
    respond: catalogue([product({}, { tax_behavior: 'unspecified', currency: 'eur' })]),
  })
  const res = await post(app, cookie, '/tiers', { label: 'S3', providerProductId: 'prod_s3' })
  ...
  assertEquals(body.message.includes('no tax behaviour resolves for this price'), true)
...
test('an account tax default rescues a price left at "Use default", end to end', async () => {
  ...
  const listed = await jsonBody<{ products: { verification: { ok: boolean; failures: string[] } }[] }>(
  ...
  assertEquals(client.calls.some((call) => call.path === '/v1/tax/settings'), true)
})
...
test('POST /tiers refuses a product the provider does not know with our words, never the provider\'s', async () => {
  ...
  const res = await post(app, cookie, '/tiers', { label: 'S3', providerProductId: 'prod_nope' })
  assertEquals(res.status, 400)
  const body = await jsonBody<{ error: string; status: number; message: string }>(res)
  assertEquals(body.error, 'product_lookup_failed')
  assertEquals(body.status, 404)
  assertEquals(body.message, 'the provider has no product with that id on this key')
...
test('POST /tiers answers 409 tier_exists for a label that already has a row, before spending a provider call', async () => {
  ...
  const res = await post(app, cookie, '/tiers', { label: 's3', providerProductId: 'prod_other' })
...
test('POST /tiers writes a conforming product with rank and isCustom from the ladder and the price cached from the provider', async () => {
  const { app, cookie, db, client } = await buildApp({
    respond: catalogue([product({ id: 'prod_s4', name: 'S4' }, { id: 'price_s4', unit_amount: 1500 })]),
  })
  const res = await post(app, cookie, '/tiers', { label: 'S4', providerProductId: 'prod_s4' })
  assertEquals(res.status, 201)
...
test('PATCH /tiers/:id re-verifies only when the product moved, and caches the new price when it did', async () => {
  const { app, cookie, client, db } = await buildApp({
    ...
    respond: catalogue([
      product(),
      product({ id: 'prod_other', name: 'S3 v2' }, { id: 'price_other', unit_amount: 1200 }),
    ]),
  })
  // Same product, and a flag-only patch: the provider is not consulted.
  const same = await patch(app, cookie, `/tiers/${S3_ID}`, { providerProductId: 'prod_s3' })
  ...
  const moved = await patch(app, cookie, `/tiers/${S3_ID}`, { providerProductId: 'prod_other', isActive: true })
  ...
  assertEquals(body.tier.priceCents, 1200)
  assertEquals(body.tier.isActive, true)
  assertEquals(client.calls.map((call) => call.path), ['/v1/products/prod_other'])
  const [row] = db.rows<TierRow>(tier)
  assertEquals(row?.providerProductId, 'prod_other')
  assertEquals(row?.priceCents, 1200)
  assertEquals(row?.currency, 'usd')
})
...
test('PATCH /tiers/:id refuses a product that does not verify and leaves the row alone', async () => {
  const { app, cookie, db } = await buildApp({
    tiers: [tierRow()],
    respond: catalogue([product({ id: 'prod_bad', name: 'Bad' }, { id: 'price_bad', active: false })]),
  })
  const res = await patch(app, cookie, `/tiers/${S3_ID}`, { providerProductId: 'prod_bad', isActive: false })
  assertEquals(res.status, 400)
  assertEquals((await jsonBody<{ error: string }>(res)).error, 'product_verification_failed')
...
test('PATCH /tiers/:id refuses a product on SX, a null product on a priced row, and an unknown field', async () => {
  const { app, cookie, client, db } = await buildApp({ tiers: [tierRow(), customRow()] })
  ...
  for (const [name, id, body] of attempts) {
    ...
    assertEquals(res.status, 400, name)
    assertEquals((await jsonBody<{ error: string }>(res)).error, 'tier_invalid', name)
  }
  assertEquals(client.calls.length, 0)
  assertEquals(db.rows<TierRow>(tier).find((row) => row.id === S3_ID)?.providerProductId, 'prod_s3')
  assertEquals(db.rows<TierRow>(tier).find((row) => row.id === SX_ID)?.providerProductId, null)

  const missing = await patch(app, cookie, '/tiers/99999999-9999-4999-8999-999999999999', { isActive: false })
...
test('POST /tiers/:id/deactivate retires the row and never deletes it', async () => {
  ...
  const missing = await post(app, cookie, '/tiers/99999999-9999-4999-8999-999999999999/deactivate')
  assertEquals(missing.status, 404)
})
...
test('POST /tiers/:id/verify refreshes the cached price from the provider', async () => {
  const { app, cookie, db, client } = await buildApp({
    ...
    respond: catalogue([product({}, { unit_amount: 1100 })]),
  })
  const res = await post(app, cookie, `/tiers/${S3_ID}/verify`)
  assertEquals(res.status, 200)
  const body = await jsonBody<{ verification: { ok: boolean }; tier: SerializedTier }>(res)
  assertEquals(body.verification.ok, true)
  assertEquals(body.tier.priceCents, 1100)
  assertEquals(db.rows<TierRow>(tier)[0]?.priceCents, 1100)
...
test('POST /tiers/:id/verify refuses a custom row with tier_has_no_product', async () => {
  ...
  assertEquals(res.status, 400)
  assertEquals((await jsonBody<{ error: string }>(res)).error, 'tier_has_no_product')
  assertEquals(client.calls.length, 0)

  const missing = await post(app, cookie, '/tiers/99999999-9999-4999-8999-999999999999/verify')
  assertEquals(missing.status, 404)
})
...
test('POST /tiers/verify covers every priced row, skips SX, and refreshes each cached price', async () => {
  ...
  const { app, cookie, db, client } = await buildApp({
    tiers: [
      ...
      tierRow({ id: S4_ID, label: 'S4', rank: 4, providerProductId: 'prod_s4', priceCents: 1500 }),
      tierRow({ id: S5_ID, label: 'S5', rank: 5, providerProductId: 'prod_gone', priceCents: 2000 }),
      customRow(),
    ],
    respond: catalogue([
      product(),
      product({ id: 'prod_s4', name: 'S4' }, { id: 'price_s4', unit_amount: 1500, billing_scheme: 'tiered' }),
    ]),
  })
  const res = await post(app, cookie, '/tiers/verify')
  assertEquals(res.status, 200)
  const body = await jsonBody<{
    results: { id: string; label: string; ok: boolean; failures: string[]; product: { id: string } | null }[]
  }>(res)
  ...
  assertEquals(s5?.failures, ['the provider has no product with that id on this key'])
