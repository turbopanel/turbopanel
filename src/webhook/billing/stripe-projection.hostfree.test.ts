...
const priceOf = (label: string) => ({ id: `price_${label}`, product: `prod_${label}` })
...
const serverRow = (id: string, assignedTierId: string | null = null) => ({
  id, organizationId: ORG, createdAt: NOW, updatedAt: NOW, metadata: null, assignedTierId,
})
...
function stripeSubscription(overrides: Obj = {}, customerOverrides: Obj = {}): Obj {
  return {
    ...
    items: {
      object: 'list',
      has_more: false,
      data: [{ id: 'si_1', object: 'subscription_item', quantity: 2, price: priceOf('S1') }],
    },
    ...overrides,
  }
}
...
function stripeProduct(label: string, unitAmount: number, overrides: Obj = {}): Obj {
  return {
    id: `prod_${label}`,
    object: 'product',
    active: true,
...
function refreshed(outcome: StripeProjectionOutcome): string[] {
  if (outcome.action !== 'catalogue_refreshed') throw new Error(`expected a catalogue refresh, got ${JSON.stringify(outcome)}`)
  return outcome.tierIds
}

function synced(outcome: StripeProjectionOutcome) {
  const entitlements = projected(outcome).entitlements
  if (entitlements?.action !== 'synced') throw new Error(`expected an entitlement sync, got ${JSON.stringify(entitlements)}`)
  return entitlements
}

function seatRows(db: MemoryDb) {
  return db.rows(subscriptionItem).map((row) => [row.providerItemId, row.tierId, row.quantity, row.providerPriceId]).sort()
}
...
test('T6 · an organization projection syncs entitlements: the licensed server is assigned, the lease is released, nothing is landed or revoked', async () => {
  const db = emptyDb({ licenses: [licenseRow(LICENSE_BOUND, SERVER)], servers: [serverRow(SERVER)] })
  const client = routedClient({ [SUB_ROUTE]: () => stripeSubscription() })
  const entitlements = synced(await projectSubscriptionById({ db, client, now: NOW }, 'sub_1'))
...
test('T6 · an ended subscription revokes every license the organization holds — bound ones included — and clears the assignment', async () => {
  ...
  const entitlements = synced(await projectSubscriptionById({ db, client, now: NOW }, 'sub_1'))
  assertEquals([...entitlements.result.revokedLicenseIds].sort(), [LICENSE_BOUND, LICENSE_SPARE].sort())
    ...
    [SUB_ROUTE]: () =>
      stripeSubscription({
        items: {
          ...
          has_more: false,
          data: [
            { id: 'si_1', object: 'subscription_item', quantity: 2, price: priceOf('S1') },
            { id: 'si_unknown', object: 'subscription_item', quantity: 1, price: { id: 'price_other', product: 'prod_other' } },
            { id: 'si_bare', object: 'subscription_item', quantity: 1, price: 'price_S2' },
          ],
        },
      }),
  })
    ...
    [SUB_ROUTE]: () =>
      stripeSubscription({
        items: {
          ...
          data: [
            { id: 'si_1', object: 'subscription_item', quantity: 2, price: priceOf('S1') },
            { id: 'si_1b', object: 'subscription_item', quantity: 3, price: { id: 'price_S1_legacy', product: 'prod_S1' } },
          ],
        },
      }),
  })
...
test('T7 · current_period_end: on the subscription pre-basil, the latest item from basil on, the subscription when both', async () => {
  ...
  const item = (id: string, end?: number) => ({ id, object: 'subscription_item', quantity: 1, price: priceOf('S1'), ...(end === undefined ? {} : { current_period_end: end }) })
...
test('T7 · every subscription event — pending_update_expired included — is one refetch of the subscription it names', async () => {
  ...
  for (const type of types) {
    const db = emptyDb()
    const client = routedClient({ [SUB_ROUTE]: () => stripeSubscription() })
    const outcome = projected(await projectStripeEvent({ db, client, now: NOW }, { id: 'evt', type, objectId: 'sub_1', objectType: 'subscription' }))
    assertEquals(outcome.subscriptionId, db.rows(subscription)[0]?.id, type)
    assertEquals(client.calls.map((c) => `${c.method} ${c.path}`), ['GET /v1/subscriptions/sub_1'], type)
...
test('T7 · items with has_more are walked through the list endpoint, and the page — not the embedded list — is what lands', async () => {
  ...
  const client = routedClient({
    [SUB_ROUTE]: () =>
      stripeSubscription({
        items: { object: 'list', has_more: true, data: [{ id: 'si_1', object: 'subscription_item', quantity: 2, price: priceOf('S1') }] },
      }),
    'GET /v1/subscription_items': () => ({
      object: 'list',
      data: [
        { id: 'si_1', object: 'subscription_item', quantity: 5, price: priceOf('S1') },
        { id: 'si_2', object: 'subscription_item', quantity: 1, price: priceOf('S2') },
      ],
    }),
  })
  projected(await projectSubscriptionById({ db, client, now: NOW }, 'sub_1'))
  ...
  assertEquals(seatRows(db), [['si_1', S1, 5, 'price_S1'], ['si_2', S2, 1, 'price_S2']])
})
...
test('T7 · schedule id or object, a missing quantity, and the first tax id all project as documented', async () => {
  const cases: [string, Obj, Obj, { scheduleId: string | null; quantity: number; taxId: string | null }][] = [
    ...
    ['quantity absent → 1', { items: { object: 'list', has_more: false, data: [{ id: 'si_1', object: 'subscription_item', price: priceOf('S1') }] } }, {}, { scheduleId: null, quantity: 1, taxId: 'DE123456789' }],
...
test('product.updated refetches the product with its default price and refreshes the tier that names it; an unknown product is skipped before any call', async () => {
  ...
  const deps = { db, client, now: NOW }
  assertEquals(refreshed(await projectStripeEvent(deps, { id: 'evt', type: 'product.updated', objectId: 'prod_S1', objectType: 'product' })), [S1])
  ...
  assertEquals(skipped(await projectStripeEvent(deps, { id: 'evt', type: 'product.updated', objectId: 'prod_nope', objectType: 'product' })), 'product_not_a_tier')
...
test('price.updated reads the price for its product only, then refreshes from the product — a retired price never overwrites the cache', async () => {
  ...
  const client = routedClient({
    ...
    'GET /v1/prices/price_S2_old': () => ({ id: 'price_S2_old', object: 'price', active: false, unit_amount: 999, product: 'prod_S2' }),
    'GET /v1/products/prod_S2': () => stripeProduct('S2', 2500),
    'GET /v1/prices/price_orphan': () => ({ id: 'price_orphan', object: 'price', product: null }),
    'GET /v1/prices/price_other': () => ({ id: 'price_other', object: 'price', product: { id: 'prod_other', object: 'product' } }),
  })
  const deps = { db, client, now: NOW }
  assertEquals(refreshed(await projectStripeEvent(deps, { id: 'evt', type: 'price.updated', objectId: 'price_S2_old', objectType: 'price' })), [S2])
  assertEquals(client.calls.map((c) => `${c.method} ${c.path}`), ['GET /v1/prices/price_S2_old', 'GET /v1/products/prod_S2'])
  assertEquals(formOf(client.calls[1]!, 'expand[0]'), 'default_price')
  assertEquals(db.rows(tier).find((row) => row.id === S2)?.priceCents, 2500)

  assertEquals(skipped(await projectStripeEvent(deps, { id: 'evt', type: 'price.updated', objectId: 'price_orphan', objectType: 'price' })), 'product_missing')
  // An expanded product on the price is read for its id like any id-or-object field.
  assertEquals(skipped(await projectStripeEvent(deps, { id: 'evt', type: 'price.updated', objectId: 'price_other', objectType: 'price' })), 'product_not_a_tier')
...
test('a product whose default price has no unit_amount leaves the cached price alone', async () => {
  const db = emptyDb()
  const client = routedClient({
    'GET /v1/products/prod_S1': () => stripeProduct('S1', 0, { default_price: { id: 'price_S1', object: 'price', active: true, currency: 'usd', unit_amount: null } }),
  })
  assertEquals(refreshed(await projectStripeEvent({ db, client, now: NOW }, { id: 'evt', type: 'product.updated', objectId: 'prod_S1', objectType: 'product' })), [S1])
  const s1 = db.rows(tier).find((row) => row.id === S1)
