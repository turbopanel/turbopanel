  ...
  for (const item of items) {
    ...
    if (!providerProductId) {
      logWarn(
        ...
        `subscription item ${providerItemId} carries no product; skipped`,
      );
      continue;
    }
...
export async function projectSubscriptionById(
  ...
  if (!providerCustomerId) {
    return { action: "skipped", reason: "customer_missing" };
  }
  ...
  if (subject.organizationId) {
    await clearPendingCheckout(deps.db, subject.organizationId);
...
async function subscriptionIdFromInvoice(
  ...
  const invoice = await client.get<StripeObject>(
  ...
  const details = parent && isObject(parent.subscription_details)
...
async function refreshTierCatalogue(
  ...
  if (event.type === "price.updated" && productId) {
    const price = await deps.client.get<StripeObject>(
      `/v1/prices/${encodeURIComponent(productId)}`,
    );
    productId = idOrObjectId(price.product);
  }
  if (!productId) return { action: "skipped", reason: "product_missing" };
  const tierByProduct = await mapProviderProductsToTierIds(deps.db, "stripe", [
    productId,
  ]);
  const tierId = tierByProduct.get(productId);
  if (!tierId) return { action: "skipped", reason: "product_not_a_tier" };
  const product = await resolveBillingGateway(deps.client).getProduct(
    productId,
  );
...
export async function projectStripeEvent(
  ...
  if (!event.objectId) {
    return { action: "skipped", reason: "object_id_missing" };
  }
  ...
  switch (event.type) {
    ...
    case "checkout.session.completed": {
      const session = await deps.client.get<StripeObject>(
        `/v1/checkout/sessions/${encodeURIComponent(event.objectId)}`,
      );
      const checkoutSubject = resolvePayerSubject(session.metadata);
      if (checkoutSubject?.organizationId) {
        await clearPendingCheckout(deps.db, checkoutSubject.organizationId);
      }
      subscriptionId = idOrObjectId(session.subscription);
      break;
    }
    case "invoice.paid":
    case "invoice.payment_failed":
      ...
      subscriptionId = await subscriptionIdFromInvoice(
        deps.client,
        event.objectId,
      );
      break;
    default:
      logInfo(
        STRIPE_PROJECTION_LOG_SCOPE,
        `event ${event.id} (${event.type}) not handled`,
      );
