...
import { capabilityPlanGeneration, organization, server } from '../../lib/db/schema.ts'
import {
...
async function withServerFixture(
  ...
  if (!dbUrl) {
    console.warn('Skipping capability plan records tests: TURBOPANEL_DATABASE_URL not set')
    return
  }
  ...
  try {
    ...
    await db.delete(capabilityPlanGeneration).where(eq(capabilityPlanGeneration.serverId, serverId))
...
test('recordCapabilityPlanGenerationIfChanged bumps generation when the plan changes', async () => {
  await withServerFixture(async ({ db, serverId }) => {
    ...
    const generation = await recordCapabilityPlanGenerationIfChanged(db, serverId, changedPlan)
  ...
  await withServerFixture(async ({ serverId }) => {
    ...
    try {
      ...
      const rows = await dbA
        ...
        .from(capabilityPlanGeneration)
        .where(eq(capabilityPlanGeneration.serverId, serverId))
      const rowA = rows.find((row) => row.generation === generationA)
      const rowB = rows.find((row) => row.generation === generationB)
