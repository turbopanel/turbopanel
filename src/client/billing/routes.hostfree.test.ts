...
function tierRow(id: string, label: 'S3' | 'S5', rank: number): TierRow {
  return {
    id, createdAt: NOW, updatedAt: NOW, label, rank, provider: 'stripe',
    providerProductId: `prod_${label.toLowerCase()}`, priceCents: 1000 * rank, currency: 'usd', isCustom: false, isActive: true,
  }
}
...
function stateWith(status: string | null, seats: { tierId: string; quantity: number }[]): OrganizationBillingState {
  const payerRow = { id: 'p', organizationId: ORG, userId: null, provider: 'stripe', providerCustomerId: 'cus_1', taxId: null, createdAt: NOW, updatedAt: NOW }
  if (!status) return { payer: payerRow, subscription: null, seats: [], grant: null }
  return {
    payer: payerRow,
    grant: null,
    subscription: {
      id: 's', payerId: 'p', providerSubscriptionId: 'sub_1', status, currentPeriodEnd: PERIOD_END, scheduleId: null,
      graceExpiresAt: null, pastDueSince: status === 'past_due' ? NOW : null, createdAt: NOW, updatedAt: NOW,
    },
...
function viewWith(state: OrganizationBillingState, extra: Partial<Omit<BillingOrgView, 'state'>> = {}): BillingOrgView {
  return {
    state,
...
async function buildApp(opts: HarnessOpts = {}) {
  ...
  const db = createMemoryDb([
    ...
    [payer, [{ id: 'p', organizationId: ORG, userId: null, provider: 'stripe', providerCustomerId: 'cus_1', taxId: null, createdAt: NOW, updatedAt: NOW }]],
    [setting, []],
  ], { fallback: authDb })
  const client = createStripeClientDouble((call) => {
    ...
    if (call.method === 'GET' && call.path === '/v1/products/prod_s3') return stripeProduct('S3')
    if (call.method === 'GET' && call.path === '/v1/products/prod_s5') return stripeProduct('S5')
  ...
  registerBillingRoutes(app, { secrets, runtime: 'workers', signupEnvOverride: undefined, baseUrl: BASE_URL }, {
    createClient: () => client,
    loadView: () => Promise.resolve(opts.view ?? viewWith(stateWith('active', [{ tierId: S3, quantity: 2 }]))),
    beginMutation: () => {
      leases.push('begin')
...
test('GET /billing/catalog lists active tiers from Postgres in ladder order with the ladder\'s entitlements, no Stripe call', async () => {
  ...
  const body = await res.json() as {
    tiers: { id: string; label: string; rank: number; priceCents: number; currency: string; isCustom: boolean; entitlements: Record<string, number> }[]
  }
  assertEquals(body.tiers.map((t) => [t.id, t.label, t.rank, t.priceCents, t.currency, t.isCustom]), [
    [S3, 'S3', 3, 3000, 'usd', false],
    [S5, 'S5', 5, 5000, 'usd', false],
  ])
  // Entitlements come from the in-code ladder, not from any column.
  assertEquals(body.tiers[0]!.entitlements, { maxCores: 16, maxMemoryBytes: 64 * 1024 ** 3, nicSlots: 5, driveSlots: 6, gpuSlots: 2, filesystemSlots: 9 })
...
test('GET /billing/subscription summarises the projection: payer, subscription, tiers, licenses net of releases, servers, pending changes', async () => {
  const intent = newDeferredIntent('release-seat', { fromTierId: S3, toTierId: null, landsAt: PERIOD_END, fromQuantity: 3, nowMs: Date.parse(NOW) })
  ...
  assertEquals(summarizeLicenses(view), { purchased: 3, granted: 0, releasing: 1, held: 1, bound: 1, available: 1 })
  const { app, headers, stripeCalls } = await buildApp({ view })
  const res = await app.request('/billing/subscription', { headers })
  assertEquals(res.status, 200)
  const body = await res.json() as Record<string, unknown>
  assertEquals(Object.keys(body).sort(), ['licenses', 'payer', 'pendingChanges', 'servers', 'subscription', 'tiers'])
  assertEquals(body.payer, { taxId: null })
  assertEquals(body.subscription, { status: 'past_due', currentPeriodEnd: PERIOD_END, pastDueSince: NOW, graceExpiresAt: null, scheduleAttached: false })
  assertEquals(body.tiers, [{ tierId: S3, label: 'S3', rank: 3, purchased: 3, inUse: 1, releasing: 1, priceCents: 3000, currency: 'usd' }])
  assertEquals(body.licenses, { purchased: 3, releasing: 1, held: 1, bound: 1, available: 1 })
  assertEquals(body.servers, [{ serverId: SRV, assignedTierId: S3, requiredTier: 'S3' }])
  assertEquals(body.pendingChanges, [{ id: intent.id, kind: 'release-seat', fromTierId: S3, toTierId: null, createdAt: NOW, landsAt: PERIOD_END }])
  assertEquals(stripeCalls(), [])
})
...
test('POST /billing/checkout sends the product\'s default price from the gateway as the line item — never a price stored on the row', async () => {
  const { app, headers, db, client, stripeCalls } = await buildApp({
    ...
    routes: {
      'POST /v1/checkout/sessions': () => ({ id: 'cs_1', object: 'checkout.session', url: 'https://checkout.stripe.com/c/pay/cs_1' }),
    },
  })
  const res = await app.request('/billing/checkout', { method: 'POST', headers, body: JSON.stringify({ tierId: S3, quantity: 2 }) })
  assertEquals(res.status, 200)
  assertEquals(await res.json(), { url: 'https://checkout.stripe.com/c/pay/cs_1', sessionId: 'cs_1' })
...
test('checkout is refused 409 subscription_exists once a live subscription is projected, and 400 tier_not_purchasable for a retired tier', async () => {
  ...
  const retired = await fresh.app.request('/billing/checkout', { method: 'POST', headers: fresh.headers, body: JSON.stringify({ tierId: S5 }) })
  assertEquals(retired.status, 400)
  assertEquals(await retired.json(), { error: 'tier_not_purchasable', reason: 'inactive', failures: [] })
...
test('POST /billing/preview accepts { tierId, delta } and pins a proration date on the invoice preview', async () => {
  const { app, headers, client, stripeCalls } = await buildApp({
    routes: { 'POST /v1/invoices/create_preview': () => ({ object: 'invoice', currency: 'usd', subtotal: 500, total: 500, amount_due: 500, lines: { data: [] } }) },
  })
  const res = await app.request('/billing/preview', { method: 'POST', headers, body: JSON.stringify({ tierId: S3, delta: 1 }) })
  assertEquals(res.status, 200)
  ...
  assertEquals(formOf(preview, 'subscription_details[proration_date]'), String(Math.floor(Date.parse(NOW) / 1000)))
})
...
test('POST /billing/preview accepts { fromTierId, toTierId } as a −1/+1 swap, resolving the target\'s price through the gateway', async () => {
  const { app, headers, client, stripeCalls } = await buildApp({
    routes: { 'POST /v1/invoices/create_preview': () => ({ object: 'invoice', currency: 'usd', subtotal: 1234, total: 1234, amount_due: 1234, lines: { data: [] } }) },
  })
  const res = await app.request('/billing/preview', { method: 'POST', headers, body: JSON.stringify({ fromTierId: S3, toTierId: S5 }) })
  assertEquals(res.status, 200)
...
test('POST /billing/preview answers 400 on an empty body, a zero delta, half of each shape, and a bad id', async () => {
  const { app, headers, stripeCalls } = await buildApp()
  for (const body of ['{}', JSON.stringify({ tierId: S3, delta: 0 }), JSON.stringify({ fromTierId: S3, delta: 1 }), JSON.stringify({ tierId: S3, toTierId: S5 }), JSON.stringify({ tierId: 'nope', delta: 1 }), 'nope']) {
    const res = await app.request('/billing/preview', { method: 'POST', headers, body })
...
test('POST /billing/seats validates the body: a tier id, a non-zero integer delta, an optional integer proration date', async () => {
  ...
  for (const body of ['{}', JSON.stringify({ tierId: S3 }), JSON.stringify({ tierId: S3, delta: 0 }), JSON.stringify({ tierId: S3, delta: 1.5 }), JSON.stringify({ tierId: 'x', delta: 1 }), JSON.stringify({ tierId: S3, delta: 1, prorationDate: 'soon' }), 'nope']) {
    const res = await app.request('/billing/seats', { method: 'POST', headers, body })
    assertEquals(res.status, 400, body)
  }
...
test('POST /billing/upgrade validates { fromTierId, toTierId } and forwards to upgradeTier, which decides the direction', async () => {
  ...
  for (const body of ['{}', JSON.stringify({ fromTierId: S3 }), JSON.stringify({ toTierId: S5 }), JSON.stringify({ fromTierId: 'not-a-uuid', toTierId: S5 }), JSON.stringify({ fromTierId: S3, toTierId: 12 }), JSON.stringify({ fromTierId: S3, toTierId: S5, prorationDate: 'x' })]) {
    const res = await app.request('/billing/upgrade', { method: 'POST', headers, body })
    assertEquals(res.status, 400, body)
    assertEquals(await res.json(), { error: 'Invalid request' }, body)
  }
  // A well-formed body reaches the mutation: only it knows S5 → S3 is not an upgrade.
  const wrongWay = await app.request('/billing/upgrade', { method: 'POST', headers, body: JSON.stringify({ fromTierId: S5, toTierId: S3 }) })
  assertEquals(wrongWay.status, 400)
  assertEquals(await wrongWay.json(), { error: 'not_an_upgrade' })
  const same = await app.request('/billing/upgrade', { method: 'POST', headers, body: JSON.stringify({ fromTierId: S3, toTierId: S3 }) })
...
test('POST /billing/downgrade validates { fromTierId, toTierId } and forwards to downgradeTier: the intent and the schedule phase come back', async () => {
  const { app, headers, leases, client, stripeCalls } = await buildApp({
    ...
    routes: {
      'POST /v1/subscription_schedules': () => ({
        id: 'sub_sched_1', object: 'subscription_schedule', status: 'active', subscription: 'sub_1',
        phases: [{ start_date: 1_700_000_000, end_date: 1_702_592_000, items: [{ price: 'price_s5', quantity: 1 }] }],
      }),
      'POST /v1/subscription_schedules/sub_sched_1': () => ({ id: 'sub_sched_1', object: 'subscription_schedule', status: 'active', phases: [] }),
    },
  })
  for (const body of ['{}', JSON.stringify({ fromTierId: S5 }), JSON.stringify({ fromTierId: 'bad', toTierId: S3 })]) {
    const res = await app.request('/billing/downgrade', { method: 'POST', headers, body })
    assertEquals(res.status, 400, body)
    assertEquals(await res.json(), { error: 'Invalid request' }, body)
  }
  const wrongWay = await app.request('/billing/downgrade', { method: 'POST', headers, body: JSON.stringify({ fromTierId: S3, toTierId: S5 }) })
  assertEquals(wrongWay.status, 400)
  assertEquals(await wrongWay.json(), { error: 'not_a_downgrade' })
  assertEquals(leases, [])

  const res = await app.request('/billing/downgrade', { method: 'POST', headers, body: JSON.stringify({ fromTierId: S5, toTierId: S3 }) })
  assertEquals(res.status, 200)
  const body = await res.json() as { ok: boolean; deferred: boolean; intentId: string; scheduleId: string }
  ...
  assertEquals(stripeCalls(), ['GET /v1/products/prod_s3', 'POST /v1/subscription_schedules', 'POST /v1/subscription_schedules/sub_sched_1'])
...
test('POST /billing/portal is 404 for an organization with no projected payer, before any Stripe call', async () => {
  const view = viewWith({ payer: null, subscription: null, seats: [], grant: null })
  const { app, headers, stripeCalls } = await buildApp({ view })
