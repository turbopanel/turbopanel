 ...
 * route when customer billing is not operational (both Stripe secrets
 * required — see `isCustomerBillingOperational`). Self-hosted has no
...
async function authenticate(c: Ctx): Promise<Authed | Response> {
  ...
  if (!isCustomerBillingOperational(config)) {
    return c.json({ error: BILLING_NOT_CONFIGURED_ERROR }, 503);
  }
  return {
    db,
...
function readTierMove(
  ...
  const fromTierId = readUuidField(body, "fromTierId");
  const toTierId = readUuidField(body, "toTierId");
  const prorationDate = withProration
    ? readIntField(body, "prorationDate")
    : null;
  if (
...
export function registerBillingRoutes(
  ...
  async function underLease(
    ...
    if (!lock) {
      return c.json({ error: BILLING_MUTATION_IN_PROGRESS_ERROR }, 409);
    }
    try {
  ...
  router.post("/billing/checkout", async (c) => {
    ...
    if (
      tierId === PARSE_UUID_INVALID || !tierId || quantity === "invalid" ||
      quantity < 1
    ) {
      return c.json({ error: "Invalid request" }, 400);
    }
    ...
    return await underLease(c, auth, async () => {
      ...
      if (hasLiveSubscription(view)) {
        return c.json({ error: SUBSCRIPTION_EXISTS_ERROR }, 409);
      }
      ...
      if (pending) {
        if (pendingCheckoutMatches(pending, { tierId, quantity })) {
          return c.json({ url: pending.url, sessionId: pending.sessionId });
        }
        return c.json({ error: CHECKOUT_PENDING_ERROR }, 409);
      }

      const client = createClient(auth.config);
      try {
        const price = await resolveTierPrice(
          auth.db,
          resolveBillingGateway(client),
          tierId,
        );
        if (!price.ok) {
          return c.json({
        ...
        const { providerCustomerId } = await ensureCustomerForOrganization(
          auth.db,
          client,
          {
  ...
  router.post("/billing/downgrade", async (c) => {
    ...
    return await answer(
      c,
      () =>
        downgradeTier(mutationDeps(auth), {
          organizationId: auth.organizationId,
          fromTierId: move.fromTierId,
          toTierId: move.toTierId,
        }),
    );
  });
}
